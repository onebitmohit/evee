...
import { consumeTelegramLinkCode } from "../db/workspaces";
...
export const telegramCommands = [
  ...
  { command: "link", description: "Connect this Telegram account to Evee" },
...
export function createTelegramBot(token: string) {
  ...
  bot.command("link", async (ctx) => {
    ...
    if (!code) {
      await ctx.reply("Open Integrations in the Evee dashboard, generate a one-time code, then send /link followed by that code.");
      return;
    }
    try {
      ...
      await ctx.reply("Connected. This chat now shares its profile, monitors, opportunities, feedback, and settings with your Evee workspace.");
    } catch (error) {
      await ctx.reply(error instanceof Error ? error.message : "I could not use that link code.");
    }
  });
  ...
  bot.command("help", async (ctx) => {
    await ctx.reply(`<b>Evee controls</b>\n/start - begin\n/link - connect this chat to your web workspace\n/setup - rebuild your business profile\n/profile - view what I learned\n/scan - scan now\n/digest - show the current digest\n/settings - set digest time and score threshold\n/pause · /resume - control alerts\n/help - show these commands\n\nUse the buttons under each alert to teach me what is useful. I only draft replies; I never post them.`, { parse_mode: "HTML" });
  });
