...
export async function analyzeOpportunity(
  ...
  if (!env.AI_GATEWAY_API_KEY) return heuristicAnalysis(candidate, profile);
  ...
  try {
    const result = await generateText({
      ...
      output: Output.object({
        ...
        description: "A scored product opportunity and a safe, useful public reply draft.",
      }),
      system: `You identify high-intent, ethical product opportunities in public conversations.
...
The component scores must sum to the total score.`,
...
export async function rewriteReply(candidate: Candidate, profile: Profile, currentDraft: string, instruction = "Make it more useful and natural") {
  ...
  const result = await generateText({
    model: env.AI_MODEL,
    system: "Rewrite a public community reply. Be helpful before promotional, concise, specific, honest, and disclose product affiliation. Return only the reply.",
    prompt: `Style: ${profile.replyStyle}\nProduct: ${profile.productName} — ${profile.productSummary}\nConversation: ${candidate.title}\nCurrent draft: ${currentDraft}\nInstruction: ${instruction}`,
  });
