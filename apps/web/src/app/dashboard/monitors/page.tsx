...
export default async function MonitorsPage() {
  ...
  return (
    <div className="grid gap-5">
      <PageHeader title="Monitors" description="Define where Evee searches and what signals matter. The copilot can turn a plain-English goal into a focused monitor." actions={<Link href="/dashboard/agents" className="flex h-8 items-center gap-2 rounded-[7px] border bg-[var(--surface)] px-3 text-[11px] font-semibold hover:border-[var(--border-strong)]">Build with AI <ArrowRight size={13} /></Link>} />
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.5fr)_minmax(300px,0.5fr)]">
        <section className="rounded-[10px] border bg-[var(--surface)]">
          <div className="border-b px-4 py-3.5"><h2 className="text-xs font-semibold">Source monitors</h2><p className="mt-0.5 text-[10px] text-[var(--text-faint)]">Enabled monitors are scanned by Trigger.dev schedules.</p></div>
          {monitors.length ? <div className="divide-y">{monitors.map((monitor) => (
            <div key={monitor.id} className="grid grid-cols-[28px_minmax(0,1fr)_auto] items-center gap-3 px-4 py-3.5 hover:bg-[var(--surface-subtle)]">
              ...
              <div className="min-w-0"><div className="flex items-center gap-2"><p className="truncate text-xs font-medium">{monitor.name}</p><span className="font-mono text-[8px] uppercase text-[var(--text-faint)]">{monitor.type}</span></div><p className="mt-1 truncate text-[10px] text-[var(--text-muted)]">{String(monitor.config.query ?? monitor.config.url ?? "Uses business profile keywords")}</p></div>
              <form action={toggleMonitor}><input type="hidden" name="sourceId" value={monitor.id} /><input type="hidden" name="enabled" value={String(!monitor.enabled)} /><button className={`rounded-[6px] px-2 py-1 text-[9px] font-semibold ${monitor.enabled ? "bg-[color-mix(in_srgb,var(--success)_12%,transparent)] text-[var(--success)]" : "bg-[var(--surface-subtle)] text-[var(--text-muted)]"}`}>{monitor.enabled ? "Active" : "Paused"}</button></form>
            </div>
        ...
        <form action={addMonitor} className="h-fit rounded-[10px] border bg-[var(--surface)] p-4">
          ...
          <div className="mt-4 grid gap-3.5">
            <label className="grid gap-1.5 text-[10px] font-semibold">Name<input name="name" required className="h-9 rounded-[7px] border bg-[var(--background)] px-3 text-xs font-normal" placeholder="Competitor alternatives" /></label>
            ...
            <label className="grid gap-1.5 text-[10px] font-semibold">Query or feed URL<textarea name="query" required className="min-h-20 rounded-[7px] border bg-[var(--background)] px-3 py-2.5 text-xs font-normal leading-5" placeholder="looking for an alternative to..." /></label>
            <button className="h-9 rounded-[7px] bg-[var(--accent)] text-[10px] font-semibold text-[var(--accent-foreground)] hover:bg-[var(--accent-hover)] active:translate-y-px">Create monitor</button>
          </div>
        </form>
      </div>
    </div>
